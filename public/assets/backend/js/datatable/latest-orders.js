$(document).ready(function () {
    var latest_table = $('#latest-orders-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
                url: APP_BACKEND_URL + '/order', // Base URL for Orders
                data: function (d) {
                    d.status_filter = $('.select-status-filter.active').val() || '';
                    d.latest_records = 1; // Pass the selected locale
                },
     ...
     $('#latest-orders-table').on('change', '.order-checkbox', function() {
    ...
    $('#searchTableList').on('keyup', function () {
        latest_table.search(this.value).draw();
    });
